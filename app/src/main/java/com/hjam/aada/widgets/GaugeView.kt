...
class GaugeView : View {
    ...
    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int)
        ...
        needle = BitmapFactory.decodeResource(
            resources,
            R.drawable.needle_big_ol, options
        )
        gaugeBack = BitmapFactory.decodeResource(
            resources,
            R.drawable.gauge_back_tr, options
        )
    ...
    fun setStyle(
        _max: Float,
        _drawArc: Boolean,
        _arcGreenMaxVal: Float,
        _arcYellowMaxVal: Float,
        _arcRedMaxVal: Float,
        _gaugeValueUnitText: String
    ) {
        maxValue = _max
        drawArc = _drawArc
        arcGreenMaxVal = _arcGreenMaxVal
        arcYellowMaxVal = _arcYellowMaxVal
        arcRedMaxVal = _arcRedMaxVal
        gaugeValueUnitText = _gaugeValueUnitText
        invalidate()
    }
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        canvas?.drawText(
            gValueText,
            xc - (gValueText.length * paintTexT.textSize / 3.5f),
            yc * 1.7F,
            paintTexT
        )
        canvas?.drawText(
            gaugeValueUnitText,
            xc - (gaugeValueUnitText.length * paintTexT.textSize / 3.5f),
            yc * 0.8f,
            paintTexT
        )
        ...
        canvas?.drawBitmap(
            needle,
            (gaugeBack.width / 2F) - 30,
            (gaugeBack.height / 2F) - 90,
            paintLine
        )
    }
